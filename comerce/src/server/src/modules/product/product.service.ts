...
import { getCachedOrFetch, invalidateNamespace } from "@/shared/utils/queryCache";

const PRODUCT_SEARCH_CACHE_NAMESPACE = "products:search";
...
export class ProductService {
  ...
  async getAllProducts(queryString: Record<string, any>) {
    const { data, cacheHit } = await getCachedOrFetch(
      PRODUCT_SEARCH_CACHE_NAMESPACE,
      queryString,
      async () => {
  ...
  async createProduct(data: {
    ...
    variants.forEach((variant, index) => {
      variant.attributes.forEach((attr, attrIndex) => {
        const value = existingValues.find((v:any) => v.id === attr.valueId);
    ...
    if (existingSkus.length > 0) {
      throw new AppError(
        400,
        `Duplicate SKUs detected: ${existingSkus.map((s:any) => s.sku).join(", ")}`
      );
    }
    ...
    return prisma.$transaction(async (tx:any) => {
      ...
      await invalidateNamespace(PRODUCT_SEARCH_CACHE_NAMESPACE);
      return product;
    });
  }
    ...
    if (variants) {
      ...
      if (categoryId) {
        ...
        requiredAttributeIds = requiredAttributes.map(
          (attr: { attributeId: string }) => attr.attributeId
        );
      }
  ...
  async bulkCreateProducts(file: Express.Multer.File) {
    ...
    if (categoryIds.length > 0) {
      ...
      const validCategoryIds = new Set(existingCategories.map((c:any) => c.id));
